...
{
	if( mName.CompareTo("refmult", TString::kIgnoreCase) == 0 && (mParameterIndex>=30 && mParameterIndex<=35) )//Run18 27 GeV MB, cut curve parameters are from fit
	{
	...
	else if( mName.CompareTo("refmult", TString::kIgnoreCase) == 0 && mParameterIndex==36 )  // isobar data run18
	{
		double NBTOFMult_fit = 0;
		if(ntofmatch<14){
		        NBTOFMult_fit = 1.7770 + 2.243*ntofmatch;
		}
		else{
		        NBTOFMult_fit = 32.653 + 0.972*ntofmatch;
		}
		if(refmult>=NBTOFMult_fit) return false;
		return true;
	}
	else
	{
...
Int_t StRefMultCorr::setParameterIndex(const Int_t RunId)
{
	...
	/////////////////////////////////////
	// Group ID for isobar data run 18
	/////////////////////////////////////
	if( mParameterIndex==36 && mName.CompareTo("refmult", TString::kIgnoreCase) == 0 ){
		if(RunId>=19084006&&RunId<=19085318)      gID = 0;
		else if(RunId>=19085318&&RunId<=19104009) gID = 1;
	}
	else gID = 9999;  // invalid dataset
...
void StRefMultCorr::readBadRunsFromHeaderFile()
{
	...
	for(Int_t i=0; i<nBadRun_refmult_isobar_2018; i++)
	{
		mBadRun.push_back(badrun_refmult_isobar_2018[i]);
	}
	
	cout<<"read in nBadRun_refmult_isobar_2018: "<<nBadRun_refmult_isobar_2018<<endl;
